...
class UpdatePasswordViewController: UIViewController,WebServiceDelegate
{
    var user_id : Any? = nil
    
    let savedataInstance = SaveDataClass.sharedInstance
    @IBOutlet  var txtOldPasswordonUpdatePassword: UITextField!
    ...
    func hitUpdatePasswordApi()
    {
        ...
        params.setValue(txtOldPasswordonUpdatePassword.text!, forKey: "old_password")
    ...
    @IBAction func updatePasswordAction(_ sender: Any)
    {
        ...
        if txtOldPasswordonUpdatePassword.text?.isEmpty == true
        {
        SVProgressHUD.showError(withStatus: "Please enter Old password.")
        }
        else if (txtOldPasswordonUpdatePassword.text!.count < 6)
        {
        SVProgressHUD.showError(withStatus: "Old Password should be atleast six characters long.")
        }
        else if txtPasswordonUpdatePassword.text?.isEmpty == true {
